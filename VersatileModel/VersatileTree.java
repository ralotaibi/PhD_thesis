...
/**
 * An adaptation to REPTree tree in Weka which is that handles 3 types of context changes: linear, non-linear and no change.  It compute the percentile in every node.
 ...
 * 
 */
public class VersatileTree extends AbstractClassifier 
...
{	
  ...
  protected   double num_train;
  /** Number of deployment examples */
  protected   double num_deploy;
  /** Training Instances */
  protected Instances train_instances;
  /** Deployment Instances */
  protected  Instances deploy_instances;
  /** Deployment Instances Indices after sorted */
  protected  int[][][] sorteddeploy;
  protected  double[][][] deployweights;
  /** The list of attributes  */
  protected boolean [] AttributeShifted;
  /** The using the percentile of attributes  */
  protected boolean [] AttributePercentile;
  /** Correcting the threshold of attributes  */
  protected boolean [] AttributeCorrectedThreshold;
  protected double []AttributeAlpha;
  protected double []AttributeBeta;
  protected double []m_class;
  /** Root instances for training */
  protected int[][][] global_indices_training;
  /** Root instances for deployment */
  protected int[][][] global_indices_deployment;
  ...
  {
    return  "Multi-label decision trees for both classification and regression tree";
  }
  ...
  protected class Tree implements Serializable, RevisionHandler 
    ...
    protected Tree[] mt_Successors;   
    /** The subtrees of this tree for deployment data. */ 
    protected Tree[] md_Successors; 
    /** The attribute to split on. */
    protected int m_Attribute = -1;
    /** The previous attribute before leaf . */
    protected int p_Attribute ;
    /** The split point at training. */
    protected double mt_SplitPoint = Double.NaN;    
    /** The percentile at training. */
    protected double mt_Percentile = Double.NaN;    
    /** The percentile index at training. */
    protected double mt_Percentile_index = Double.NaN;  
    /** The split point at deployment. */
    protected double md_SplitPoint = Double.NaN;    
    /** The percentile at deployment. */
    protected double md_Percentile = Double.NaN; 
    /** The percentile of the class. */
    protected double [][]mt_class_Percentile=null ;
    /** The percentile of the class. */
    protected double [][]md_class_Percentile=null ;
    /** The percentile index at deployment. */
    protected double md_Percentile_index = Double.NaN;  
    /** The proportions of training instances going down each branch. */
    protected double[]mt_Prop = null;
    /** Class probabilities from the training data in the nominal case. 
	Holds the mean in the numeric case. */
    protected double[] mt_ClassProbs = null; 
    /** The proportions of deployment instances going down each branch. */
    protected double[]md_Prop = null;
    /** Class probabilities from the deployment data in the nominal case. 
	Holds the mean in the numeric case. */
    protected double[] md_ClassProbs = null; 
    /** The (unnormalized) class distribution in the nominal
	case. Holds the sum of squared errors and the weight 
	in the numeric case. */
    protected double[] mt_Distribution = null; 
  ...
  //For reem to return more than values
    public class SplitClass
    {
        public double thr;
        public double percentile;
        public double percentile_index;    
        public double [][]class_percentile;
    }
    /**
    ...
    protected void buildTree(int[][][] sortedIndices, double[][][] weights,
      ...
      throws Exception 
    {  
      // Store structure of data set, set minimum number of instances
      ...
      if (sortedIndices[0][helpIndex].length == 0) 
      {
	    if (data.classAttribute().isNumeric()) 
	     {
	      mt_Distribution = new double[2];
	     } 
	    else 
	     {
	      mt_Distribution = new double[data.numClasses()];
	     }
	    mt_ClassProbs = null;
      ...
      mt_ClassProbs = new double[classProbs.length];
      System.arraycopy(classProbs, 0, mt_ClassProbs, 0, classProbs.length);

      if ((totalWeight < (2 * minNum)) ||
	  // Nominal case
	  (data.classAttribute().isNominal() &&
	   Utils.eq(mt_ClassProbs[Utils.maxIndex(mt_ClassProbs)],Utils.sum(mt_ClassProbs))) ||
	  // Numeric case
	  ...
	  ((m_MaxDepth >= 0) && (depth >= maxDepth))) 
      {
	     ...
	     {
	     // Nominal case
	      mt_Distribution = new double[mt_ClassProbs.length];
	      for (int i = 0; i < mt_ClassProbs.length; i++) 
	      {
	      mt_Distribution[i] = mt_ClassProbs[i];
	      }
          doSmoothing_T();
	      Utils.normalize(mt_ClassProbs);
	  } 
	 else 
	 {
	     // Numeric case
	     mt_Distribution = new double[2];
	     mt_Distribution[0] = priorVar;
	     mt_Distribution[1] = totalWeight;
	 }
        sortedIndices[0] = null;
        weights[0] = null;
	return;
      ...
      SplitClass [] objarr=new SplitClass[data.numAttributes()];
      //
      if (data.classAttribute().isNominal()) 
      { 
	   // Nominal case
	  for (int i = 0; i < data.numAttributes(); i++) 
	  {
	    ...
	    {
	    	objarr[i]=distribution(props, dists, i, sortedIndices[0][i],  weights[0][i], totalSubsetWeights, data);
	 ...
	 for (int i = 0; i < data.numAttributes(); i++) 
	 {
	   if (i != data.classIndex()) 
	   {
		    objarr[i]= numericDistribution(props, dists, i, sortedIndices[0][i], weights[0][i], totalSubsetWeights, data, vals);
	   }
	 }
      }
      // Find best attribute
      ...
      // Check if there are at least two subsets with required minimum number of instances
      int count = 0;
      ...
      if (Utils.gr(vals[m_Attribute], 0) && (count > 1))
      {
	    ...
	    mt_SplitPoint = objarr[m_Attribute].thr;
	    mt_Percentile=objarr[m_Attribute].percentile;
	    mt_Percentile_index=objarr[m_Attribute].percentile_index;
	    mt_class_Percentile=objarr[m_Attribute].class_percentile;

	    mt_Prop = props[m_Attribute];
	    ...
	    splitData(subsetIndices, subsetWeights, m_Attribute, mt_SplitPoint, sortedIndices[0], weights[0], data);
	    //Use the Percentile to Split not the threshold
        // Release memory
        sortedIndices[0] = null;
        weights[0] = null;
        // Build successors
        int temp=m_Attribute;
	    mt_Successors = new Tree[numAttVals];
	    for (int i = 0; i < numAttVals; i++) 
	    {
	     mt_Successors[i] = new Tree();
	     mt_Successors[i].p_Attribute=temp;
	     mt_Successors[i].buildTree(subsetIndices[i], subsetWeights[i], data, attTotalSubsetWeights[i],attSubsetDists[i], header, minNum,  minVariance, depth + 1, maxDepth);
  ...
  /**
  * Deploy deployment data on the trained mode. This method goes through the built tree and split the deployment data according to this tree using the percentile.
  * Note that this does not build a new tree.
  * Written by: Reem Al-Otaibi
  * @throws Exception 
  */
protected void deploy_tree(int[][][] sortedIndices, double[][][] weights,
		     Instances data, double totalWeight,double[] deployclassProbs,  Instances header,
		     double minNum, int depth, int maxDepth) 
 throws Exception 
{  
 // Make leaf if there are no training instances
 int helpIndex = 0;
 ...
 // Check if node doesn't contain enough instances, is pure or the maximum tree depth is reached
 md_ClassProbs = new double[deployclassProbs.length];
 System.arraycopy(deployclassProbs, 0, md_ClassProbs, 0, deployclassProbs.length);
 if ((totalWeight < (2 * minNum)) ||
 // Nominal case
 (data.classAttribute().isNominal() && Utils.eq(md_ClassProbs[Utils.maxIndex(md_ClassProbs)],Utils.sum(md_ClassProbs))) ||
 // Check tree depth
 ((m_MaxDepth >= 0) && (depth >= maxDepth))) 
 {
	   //Make leaf
	   m_Attribute = -1;
		...
		{
		  // Nominal case
		  if(Utils.sum(md_ClassProbs)>0)
		  {
	          doSmoothing_D();
		      Utils.normalize(md_ClassProbs);
		  }
		} 
		 for(int i=0;i<md_ClassProbs.length;i++)
sortedIndices[0] = null;
weights[0] = null;
return;
 }
...
if (m_Attribute > -1) 
{
	 ...
	 {
	    md_SplitPoint = mt_SplitPoint;
	    md_Percentile=mt_Percentile;
	    md_Percentile_index=mt_Percentile_index;
	    md_ClassProbs=mt_ClassProbs;
 	    md_class_Percentile=mt_class_Percentile;
	 }
	 else
	 { 		   
 	    md_Percentile=mt_Percentile;
 	    md_class_Percentile=mt_class_Percentile;
	    int[] inx=global_indices_deployment[0][m_Attribute];
	    int num=inx.length;
	    md_Percentile_index=(int)(md_Percentile*num)/100;
	    Instance x,y;
	    if((int)md_Percentile_index>0)
	    {
	     x = data.instance(inx[(int)(md_Percentile_index)]);
	     y = data.instance(inx[(int)(md_Percentile_index-1)]);
		 md_SplitPoint=(x.value(m_Attribute)+y.value(m_Attribute))/2;
	    }
	    else
	    {
		 x = data.instance(inx[(int)md_Percentile_index]);
	     md_SplitPoint=x.value(m_Attribute);
	    }
	 }
 }
 int numAttVals=0;
 double[][] deploydist=null;
 ...
 if (m_Attribute > -1) 
{
	 ...
	 { 
	  // Nominal case
		 deploy_distribution( sortedIndices[0][m_Attribute],weights[0][m_Attribute],totalSubsetWeights,data,m_Attribute,md_SplitPoint); 
		 numAttVals=totalSubsetWeights[m_Attribute].length;	
		 deploydist=new double[numAttVals][data.numClasses()];
	 }
	 else 
	 {
	 // Numeric case	   
		 deploy_distribution(sortedIndices[0][m_Attribute],weights[0][m_Attribute],totalSubsetWeights,data,m_Attribute,md_SplitPoint); 
		 numAttVals=2;
	     deploydist =new double[numAttVals][2];
	 }
	 int count = 0;
     for (int i = 0; i < numAttVals; i++) 
     {
     ...
     if (count > 1) 
     {
    	//Compute the deployment class distribution using the percentile
		 for(int i=0;i<numAttVals ;i++)
		 {
			 for(int j=0;j<data.numClasses();j++)
			 {
		      deploydist[i][j]=Math.ceil(md_class_Percentile[i][j]*totalWeight*m_class[j]);
			 }
		 }
	    ...
	    int[][][][] subsetIndices = new int[numAttVals][1][data.numAttributes()][0];
	    double[][][][] subsetWeights = new double[numAttVals][1][data.numAttributes()][0];
	    splitData(subsetIndices, subsetWeights, m_Attribute, md_SplitPoint, sortedIndices[0], weights[0], data);
	    int temp=m_Attribute;
	      for (int i = 0; i < numAttVals; i++) 
	      { 
		    mt_Successors[i].p_Attribute=temp;
		    mt_Successors[i].deploy_tree(subsetIndices[i], subsetWeights[i], data, attTotalSubsetWeights[i], deploydist[i],header, minNum, depth + 1, maxDepth);
	      }
    }
}
 else 
{   
	 //System.out.println("stope here 2");
     //Make leaf
	 m_Attribute = -1;
     sortedIndices[0] = null;
     weights[0] = null;
...
/**
 * Computes the split for an attribute.
 * @param sortedIndices the sorted indices of the instances
 * @param weights the weights of the instances
 ...
 * Written by: Reem Al-Otaibi
 * */
protected void deploy_distribution(int[] sortedIndices,double[] weights,double[][] subsetWeights,  Instances data, int att,double md_SplitPoint) 
	      throws Exception 
	{
	      ...
	      if (attribute.isNominal()) 
	      {
		    ...
		    dist = new double[attribute.numValues()];
		    for (i = 0; i < sortedIndices.length; i++)
		    {
		      ...
		      dist[(int)inst.value(att)] += weights[i];
		    }
	      } 
	      else 
	      {
		  ...
		  // Move all instances into second subset
		   for (i = 0; i < sortedIndices.length; i++) 
		    {
		      ...
		      if (inst.value(att) > md_SplitPoint) 
		    	  dist[0]+= weights[i];
		     else  
		    	  dist[1] += weights[i];
	        }
		   ...
		   subsetWeights[att]= new double[dist.length];
		      for (int j = 0; j < dist.length; j++) 
			   {
		    	  subsetWeights[att][j] = dist[j];
			   }
	    }
  /**
  * Computes labels class distribution of an instance using the tree.
...
protected double[] distributionForInstance(Instance instance) throws Exception 
 {
   ...
   if (m_Attribute > -1) 
   {
	 ...
	 {  
		 System.out.println("MISSING");
	      ...
	      // Split instance up
	      for (int i = 0; i < mt_Successors.length; i++)
	       {
	        double[] help =  mt_Successors[i].distributionForInstance(instance);
	        if (help != null) 
	        {
	          for (int j = 0; j < help.length; j++) 
	           {
		        returnedDist[j] += mt_Prop[i] * help[j];
	           }
	        }
	       }
    } 
	...
	{  
	  // For nominal attributes
	  returnedDist =  mt_Successors[(int)instance.value(m_Attribute)].distributionForInstance(instance);
	} 
	else 
	{    // For numeric attributes
		//Use the distribution of the deployment if the attribute is shifted
		if(AttributeShifted[m_Attribute]==true)
		{
			if(AttributeCorrectedThreshold[m_Attribute]==true)
			{
				//Use the corrected threshold
				if ((instance.value(m_Attribute)-AttributeBeta[m_Attribute])/AttributeAlpha[m_Attribute] < mt_SplitPoint) 
			      {
			        returnedDist = mt_Successors[0].distributionForInstance(instance);
			      } 
			     else 
			     {
			        returnedDist = mt_Successors[1].distributionForInstance(instance);
			     }
			...
			{
				//Use the percentile
				if (instance.value(m_Attribute) < md_SplitPoint) 
			      {
			        returnedDist = mt_Successors[0].distributionForInstance(instance);
			      } 
			     else 
		...
		//Use the distribution of the training if the attribute is not shifted
		else
		{
			if (instance.value(m_Attribute) < mt_SplitPoint) 
		      {
		        returnedDist = mt_Successors[0].distributionForInstance(instance);
		      } 
		     else 
   ...
   {
		if(AttributeShifted[p_Attribute]==true)
		{
			// Node is a leaf or successor is empty
         if (md_ClassProbs == null) 
         {                 
	               return md_ClassProbs;
         }   
	       return (double[])md_ClassProbs.clone();
		}
		else
		{
			  if (mt_ClassProbs == null) 
	           {                 
		               return mt_ClassProbs;
	           }   
		       return (double[])mt_ClassProbs.clone();
		}
   } 
   else 
   {
    ...
    /**
     * Smoothes class probabilities stored at node at training.
     */
    protected void doSmoothing_T() 
    {

      double val = m_InitialCount;
      if (m_SpreadInitialCount) 
      {
        val /= (double)mt_ClassProbs.length;
      } 
      for (int i = 0; i < mt_ClassProbs.length; i++) 
      {
        mt_ClassProbs[i] += val;
    ...
    /**
     * Smoothes class probabilities stored at node at deployment.
     */
    protected void doSmoothing_D() 
    {

      double val = m_InitialCount;
      if (m_SpreadInitialCount) 
      {
        val /= (double)md_ClassProbs.length;
      } 
      for (int i = 0; i < md_ClassProbs.length; i++) 
      {
        md_ClassProbs[i] += val;
     ...
     */
    protected void splitData(int[][][][] subsetIndices, double[][][][] subsetWeights,int att, double splitPoint, 
		          ...
		          if (mt_Prop[k] > 0) 
		           {
		            ...
		            subsetWeights[k][0][i][num[k]] = mt_Prop[k] * weights[i][j];
		            num[k]++;
		           }
		         }
	          } 
     ...
     * @throws Exception if computation fails
     */
protected SplitClass distribution(double[][] props,double[][][] dists, int att, int[] sortedIndices,double[] weights, double[][] subsetWeights,  Instances data) 
      throws Exception 
{
      SplitClass obj=new SplitClass();
      Attribute attribute = data.attribute(att);
      double[][] dist = null;
      int i;
      if (attribute.isNominal()) 
	   ...
	   for (i = 0; i < sortedIndices.length; i++) 
	    {
	      ...
	      if (inst.value(att) > currSplit) 
	     {
	      currVal = gain(currDist, priorVal);
	       ...
	       {
	          bestVal = currVal;
	          obj.thr = (inst.value(att) + currSplit) / 2.0;
	          //Global Percentile
	          int k;
	          int []d=global_indices_training[0][att];
	          for( k=0;k<d.length;k++)
	          {
	        	  double x=data.instance(d[k]).value(att);
	        	  if(x==inst.value(att))
	        	  {
	        		  break;
	        	  }
	          }
	          obj.percentile=((double)k/global_indices_training[0][att].length)*100;
	          obj.percentile_index=k;
              // Check for numeric precision problems
              if (obj.thr <= currSplit) 
              {
            	  obj.thr = inst.value(att);
            	  obj.percentile=((double)k/global_indices_training[0][att].length)*100;
      	          obj.percentile_index=k;
              }
      ...
      dists[att] = dist;
      //Compute the classes percentile
      if (attribute.isNominal()) 
      {
  	    	obj.class_percentile=new double[attribute.numValues()][data.numClasses()];//keep the percentile per class per split
  	    	double [] sum=new double[data.numClasses()];
  	      ...
  	      for( i=0;i<data.numClasses() ;i++)
	    	{
	    		...
	    		{
	    			if(sum[i]>0)
	    			obj.class_percentile[j][i]=dist[j][i]/sum[i];
	    		}
	    	}
      }
      else
      {
          obj.class_percentile=new double[2][data.numClasses()];//keep the percentile per class per split
          double [] sum=new double[data.numClasses()];
          for( i=0;i<data.numClasses() ;i++)
	    	{
	    		for( int j=0;j<2;j++)
     ...
     * @throws Exception if computation fails
     */
protected SplitClass numericDistribution(double[][] props, double[][][] dists, int att, int[] sortedIndices,double[] weights, double[][] subsetWeights, Instances data, double[] vals) 
      throws Exception 
{
      SplitClass obj=new SplitClass();
      Attribute attribute = data.attribute(att);
    ...
    //Compute the classes percentile
    	  for( i=0;i<data.numClasses();i++)
    	      for( int j=0;j<2;j++)
    		  obj.class_percentile[j][i]=dist[j][i]/Utils.sum(dist[j]);     
     ...
     */
       ...
       throws Exception 
     {
       ...
       if(mt_ClassProbs == null)
         currentProbs = parent.mt_ClassProbs;
       else
         currentProbs = mt_ClassProbs;
 	...
 	for (int i=0;i<mt_Successors.length; i++) 
 	{
 	  ...
 	  if(mt_Successors[i].m_Attribute == -1) 
 	  {
 	    double[] successorProbs = mt_Successors[i].mt_ClassProbs;
 	    if(successorProbs == null)
 	      successorProbs = mt_ClassProbs;
 	    text.append("      p = ");
 	    if(m_info.classAttribute().isNumeric()) 
 	    {
 	  ...
 	  {
 	    StringBuffer [] sub = mt_Successors[i].toSource(className, this);
 	    text.append("" + sub[0]);
             atEnd.append("" + sub[1]);
 	  }
 	  text.append("    } ");
 	  if (i == mt_Successors.length - 1) 
 	  {
 	    text.append("\n");
 	  }
         }
 	...
 	for (int i = 0; i < mt_Successors.length; i++) 
 	{
 	  text.append("N" + Integer.toHexString(Tree.this.hashCode()) 
 		      + "->" + 
 		      "N" + 
 		      Integer.toHexString(mt_Successors[i].hashCode())  +
 		      " [label=\"");
 	  if (m_info.attribute(m_Attribute).isNumeric()) 
 	  {
 	    if (i == 0) {
 	      text.append(" < " +
 			  Utils.doubleToString(mt_SplitPoint, 2));
 	    } else {
 	      text.append(" >= " +
 			  Utils.doubleToString(mt_SplitPoint, 2));
 	    }
 	  ...
 	  text.append("\"]\n");
 	  num = mt_Successors[i].toGraph(text, num, this);
 	}
       }
 	...
 	if (mt_ClassProbs == null) 
 	{
 	  classMean = parent.mt_ClassProbs[0];
 	} else {
 	  classMean = mt_ClassProbs[0];
 	}
 	StringBuffer buffer = new StringBuffer();
 	buffer.append(" : " + Utils.doubleToString(classMean, 2));
 	double avgError = 0;
 	if (mt_Distribution[1] > 0) 
 	{
 	  avgError = mt_Distribution[0] / mt_Distribution[1];
 	}
 	buffer.append(" (" +
 		      Utils.doubleToString(mt_Distribution[1], 2) + "/" +
 	...
 	if (mt_ClassProbs == null) 
 	{
 	  maxIndex = Utils.maxIndex(parent.mt_ClassProbs);
 	} else 
 	{
 	  maxIndex = Utils.maxIndex(mt_ClassProbs);
 	}
 	return " : " + m_info.classAttribute().value(maxIndex) + 
 	  " (" + Utils.doubleToString(Utils.sum(mt_Distribution), 2) + 
 	  "/" + 
 	  Utils.doubleToString((Utils.sum(mt_Distribution) - 
 				mt_Distribution[maxIndex]), 2) + ")" +
 	...
 	} else if (m_info.attribute(m_Attribute).isNominal())
 	{
 	  ...
 	  for (int i = 0; i < mt_Successors.length; i++)
 	  {
 	    ...
 	    text.append(mt_Successors[i].toString(level + 1, this));
 	  }
 	} else 
 	{
 	  ...
 	  text.append(m_info.attribute(m_Attribute).name() + " < " +
 		      Utils.doubleToString(mt_SplitPoint, 2));
 	  text.append(mt_Successors[0].toString(level + 1, this));
 	  text.append("\n");
 	  for (int j = 0; j < level; j++) 
 	  {
 	    text.append("|   ");
 	  ...
 	  text.append(m_info.attribute(m_Attribute).name() + " >= " +
 		      Utils.doubleToString(mt_SplitPoint, 2));
 	  text.append(mt_Successors[1].toString(level + 1, this));
 	}
	...
	return m_HoldOutError;
      ...
      for (int i = 0; i < mt_Successors.length; i++) 
      {
	errorTree += mt_Successors[i].reducedErrorPrune();
      }
	...
	if (mt_ClassProbs == null) 
	{
	  predictedClass = Utils.maxIndex(parent.mt_ClassProbs);
	} else {
	  predictedClass = Utils.maxIndex(mt_ClassProbs);
	}
	...
	double diff = 0;
	if (mt_ClassProbs == null) {
	  diff = parent.mt_ClassProbs[0] - inst.classValue();
	} else {
	  diff =  mt_ClassProbs[0] - inst.classValue();
	}
	...
	if (inst.isMissing(m_Attribute))
	{
	  ...
	  for (int i = 0; i < mt_Successors.length; i++) {
	    if (mt_Prop[i] > 0) {
	      mt_Successors[i].insertHoldOutInstance(inst, weight * 
						    mt_Prop[i], this);
	    }
	  }
	} else {
	  ...
	  if (m_info.attribute(m_Attribute).isNominal())
	  {
	    ...
	    mt_Successors[(int)inst.value(m_Attribute)].
	      insertHoldOutInstance(inst, weight, this);
	  } else {
	    
	    // Treat numeric attributes
	    if (inst.value(m_Attribute) < mt_SplitPoint) {
	      mt_Successors[0].insertHoldOutInstance(inst, weight, this);
	    } else {
	      mt_Successors[1].insertHoldOutInstance(inst, weight, this);
	    }
	  }
	}
      }
	...
	// Nominal case
	if (mt_ClassProbs == null) {
	  mt_ClassProbs = new double[m_info.numClasses()];
	}
	System.arraycopy(mt_Distribution, 0, mt_ClassProbs, 0, m_info.numClasses());
        for (int i = 0; i < m_HoldOutDist.length; i++) 
        {
          mt_ClassProbs[i] += m_HoldOutDist[i];
        }
        if (Utils.sum(mt_ClassProbs) > 0) 
        {
          doSmoothing_T();
          Utils.normalize(mt_ClassProbs);
        } else {
	...
	// Numeric case
        double sumOfWeightsTrainAndHoldout = mt_Distribution[1] + m_HoldOutDist[0];
        if (sumOfWeightsTrainAndHoldout <= 0) 
        {
          return;
        }
	if (mt_ClassProbs == null) {
	  mt_ClassProbs = new double[1];
	} else {
          mt_ClassProbs[0] *= mt_Distribution[1];
        }
	mt_ClassProbs[0] += m_HoldOutDist[1];
	mt_ClassProbs[0] /= sumOfWeightsTrainAndHoldout;
      }	
      ...
      if (m_Attribute != -1) {
        for (int i = 0; i < mt_Successors.length; i++)
        {
          mt_Successors[i].backfitHoldOutSet();
        }
      }
    }
  ...
  public void buildClassifier(Instances data) throws Exception 
  {
    ...
    num_train=train.numInstances();
    train_instances=train;
    // Create array of sorted indices and weights
    int[][][] sortedIndices = new int[1][train.numAttributes()][0];
    global_indices_training = new int[1][train.numAttributes()][0];
	...
	if (train.attribute(j).isNominal()) 
	{
	  ...
	  global_indices_training[0][j]= new int[train.numInstances()];
	  int count = 0;
	  for (int i = 0; i < train.numInstances(); i++) 
	  {
	    ...
	    if (!inst.isMissing(j)) 
	    {
	    	global_indices_training[0][j][count] = i;
	...
	else 
	{
	  ...
	  global_indices_training[0][j] = Utils.sort(vals);
 ...
 * @throws Exception 
   */
  public  void correct_threshold(int index, double alpha, double beta) throws Exception   
  {
	  AttributeCorrectedThreshold[index]=true;
	  AttributeAlpha[index]=alpha;
	  AttributeBeta[index]=beta;
  }
  
  public  void use_percentile(int index) throws Exception   
  {
	  AttributePercentile[index]=true;
  }

  public  void ShiftedAtt(int index) throws Exception   
  {
	  AttributeShifted[index]=true;
  }
  public  void deploy_info(Instances deploy, double []class_prop) throws Exception   
  {
	    m_class=class_prop;
	    num_deploy=deploy.numInstances();
	    deploy_instances=deploy;
	    AttributeShifted=new boolean[deploy.numAttributes()];
	    AttributeCorrectedThreshold=new boolean[deploy.numAttributes()];
	    AttributePercentile=new boolean[deploy.numAttributes()];
	    AttributeAlpha=new double[deploy.numAttributes()];
	    AttributeBeta=new double[deploy.numAttributes()];

	    for (int i=0;i<AttributeShifted.length;i++)
	    {
	    	AttributeShifted[i]=false;
	    	AttributeCorrectedThreshold[i]=false;
	    	AttributePercentile[i]=false;
	    }
	 // Create array of sorted indices and weights
	    int[][][] sortedIndices = new int[1][deploy.numAttributes()][0];
	    global_indices_deployment= new int[1][deploy.numAttributes()][0];
	    double[][][] weights = new double[1][deploy.numAttributes()][0];
	    double[] vals = new double[deploy.numInstances()];
	    for (int j = 0; j < deploy.numAttributes(); j++) 
	    {
	      if (j != deploy.classIndex()) 
	      {
	    	weights[0][j] = new double[deploy.numInstances()];
		    if (deploy.attribute(j).isNominal()) 
		   {
		  // Handling nominal attributes. Putting indices of
		  // instances with missing values at the end.
		  sortedIndices[0][j] = new int[deploy.numInstances()];
		  global_indices_deployment[0][j] = new int[deploy.numInstances()];
		  int count = 0;
		  for (int i = 0; i < deploy.numInstances(); i++) 
		  {
		    Instance inst = deploy.instance(i);
		    if (!inst.isMissing(j)) 
		    {
		      sortedIndices[0][j][count] = i;
		      global_indices_deployment[0][j][count] = i;
		      weights[0][j][count] = inst.weight();
		      count++;
		    }
		  }
		...
		else 
		{
		  ...
		  sortedIndices[0][j] = Utils.sort(vals);
		  global_indices_deployment[0][j] = Utils.sort(vals);
		  for (int i = 0; i < deploy.numInstances(); i++) 
		  {
		    weights[0][j][i] = deploy.instance(sortedIndices[0][j][i]).weight();
		  }
		}
	      }
	    }
	    ...
	    {
	      Instance inst = deploy.instance(i);
	      if (deploy.classAttribute().isNominal()) 
	      {
	    	  totalWeight += inst.weight();
	      } 
	      else 
	    ...
	    m_Tree.deploy_tree(sortedIndices, weights, deploy,totalWeight,class_prop,new Instances(deploy, 0), m_MinNum,0, m_MaxDepth);
  }
  /**
  ...
  public double[] getMembershipValues(Instance instance) throws Exception 
  {
    ...
    if (m_zeroR != null) {
      ...
      while (!queueOfNodes.isEmpty()) {
        ...
        double[] weights = new double[node.mt_Successors.length];
        if (instance.isMissing(node.m_Attribute)) {
          System.arraycopy(node.mt_Prop, 0, weights, 0, node.mt_Prop.length);
        } else if (node.m_info.attribute(node.m_Attribute).isNominal()) {
	  weights[(int)instance.value(node.m_Attribute)] = 1.0;
	} else {
	  if (instance.value(node.m_Attribute) < node.mt_SplitPoint) {
            weights[0] = 1.0;
	  } else {
            weights[1] = 1.0;
	  }
	}
        for (int i = 0; i < node.mt_Successors.length; i++) {
          queueOfNodes.add(node.mt_Successors[i]);
