...
public class BinC45ModelSelection 
{
  ...
  public double [][] featuresselection(Instances data) throws Exception
  {
      int L = data.classIndex();
      BinC45Split [] currentModel=new BinC45Split[data.numAttributes()];
      double sumOfWeights = data.sumOfWeights();;
      double [][] RandomAtt=new double[data.numAttributes()][2];

	  for (int i = L; i < data.numAttributes(); i++)
      {
          // Get models for current attribute.
          if (i != 0)
          {
          currentModel[i] = new BinC45Split(i,m_minNoObj,sumOfWeights);
          currentModel[i].buildClassifier(data);
    	  RandomAtt[i][0]=currentModel[i].attIndex();
    	  RandomAtt[i][1]=currentModel[i].Quality();
          }
      }
  ...
  public double [][] featuresselectionCC(Instances data, int m) throws Exception
  {
      BinC45Split [] currentModel=new BinC45Split[data.numAttributes()];
      double sumOfWeights = data.sumOfWeights();;
      double [][] RandomAtt=new double[data.numAttributes()][2];
